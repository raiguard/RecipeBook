	...
	for recipe_name, recipe in pairs(product_of_recipes) do
		if recipe.unlock_results then
			for technology_name, technology in pairs(game.technology_prototypes) do
				...
				then
					properties.unlocked_by[#properties.unlocked_by + 1] =
						{ type = "technology", name = technology_name }
				end
			end
		end
	end
...
local function add_entity_properties(properties, entity)
	...
	elseif entity.type == "resource" then
		...
		for _, entity in pairs(game.get_filtered_entity_prototypes({ { filter = "type", type = "mining-drill" } })) do
			if
				entity.resource_categories[resource_category] and (not required_fluid or entity.fluidbox_prototypes[1])
			then
