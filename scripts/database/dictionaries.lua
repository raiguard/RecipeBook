...
local function rebuild()
  ...
  for _, entity in pairs(prototypes.entity) do
    ...
    flib_dictionary.add("description", path, entity.localised_description) --- @diagnostic disable-line:param-type-mismatch
  end
  for _, equipment in pairs(prototypes.equipment) do
    local path = util.get_path(equipment)
    flib_dictionary.add("description", path, equipment.localised_description) --- @diagnostic disable-line:param-type-mismatch
  end
  for _, fluid in pairs(prototypes.fluid) do
    local path = util.get_path(fluid)
    flib_dictionary.add("description", path, fluid.localised_description) --- @diagnostic disable-line:param-type-mismatch
  end
  for _, item in pairs(prototypes.item) do
    local path = util.get_path(item)
    flib_dictionary.add("description", path, item.localised_description) --- @diagnostic disable-line:param-type-mismatch
  end
  for _, recipe in pairs(prototypes.recipe) do
    local path = util.get_path(recipe)
    flib_dictionary.add("description", path, recipe.localised_description) --- @diagnostic disable-line:param-type-mismatch
  end
  for _, tile in pairs(prototypes.tile) do
    local path = util.get_path(tile)
    flib_dictionary.add("description", path, tile.localised_description) --- @diagnostic disable-line:param-type-mismatch
  end
end
